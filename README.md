...
inphared.pl (**IN**frastructure for a **PHA**ge **RE**ference **D**atabase) is a perl script which downloads and filters phage genomes from Genbank to provide the most complete phage genome database available.
...
To run this script, the two additional files (`exclusion.txt` and `large_accession.txt` (rename this to something more informative?)) must be present in the current working directory. Use inphared.pl with the following command:
...
Upon first usage, it will take a long time to call genes on all of the genomes. This time can be reduced by downloading the existing `GenomesDB/` directory from HERE. Download and unzip this tar archive in the current working directory, so `GenomesDB` is a sub-directory of the current working directory.
...
And here's one we made earlier showing a DNA polymerase phylogenetic tree annotated with `15Jan2021_itol_node_label_annotations.txt`, `15Jan2021_itol_genus_annotations.txt`, `15Jan2021_itol_subfamily_annotations.txt`, `15Jan2021_itol_family_annotations.txt`, and`15Jan2021_itol_host_annotations.txt`...

INSERT BEAUTIFUL IMAGE OF CIRCLES
