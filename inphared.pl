...
#Get the number of cpus to use in Prokka as a commandline option
GetOptions(
  'cpus=i'   => \(my $cpu = 8),
);
...
my $mapping = $date."_vConTACT2_gene_to_genome.csv";
...
my $vcontact_genus = $date."_vConTACT2_genus_annotations.tsv";
...
if (! -f "$vcontact_genus") {
    ...
    #Give it some headers
    print VGENUS "Node_ID\tGenus\tColour (Genus)\n";
...
my $vcontact_sub = $date."_vConTACT2_subfamily_annotations.tsv";
...
if (! -f "$vcontact_sub") {
    ...
    #Give it some headers
    print VSUB "Node_ID\tSub-family\tColour (Sub-family)\n";
...
my $vcontact_family = $date."_vConTACT2_family_annotations.tsv";
...
if (! -f "$vcontact_family") {
    ...
    #Give it some headers
    print VFAMILY "Node_ID\tFamily\tColour (Family)\n";
...
my $vcontact_lowest_taxa = $date."_vConTACT2_lowest_taxa_annotations.tsv";
...
if (! -f "$vcontact_lowest_taxa") {
    ...
    #Give it some headers
    print VLOWEST "Node_ID\tLowest Taxa\tColour (Lowest taxa)\n";
...
my $vcontact_host = $date."_vConTACT2_host_annotations.tsv";
...
if (! -f "$vcontact_host") {
    ...
    #Give it some headers
    print VHOST "Node_ID\tHost\tColour (Host)\n";
...
if (-e "$phagedb") { 
    ...
    system ("$esearch_path -db nucleotide -query \"$meta\" | $efilter_path -query \"1417:700000 [SLEN] \" | $efetch_path -format gb >$phagedb");
    system ("$esearch_path -db nucleotide -query \"gbdiv_PHG\"[prop] | $efilter_path -query \"1417:700000 [SLEN] \" | $efetch_path -format gb >>$phagedb"); 
};

#Create output file with relevant extension for tab separated data relating to phage genomes 
my $output = $date."_data.tsv";

#And make one that excludes RefSeq sequences...
my $output_without_refseq = $date."_data_excluding_refseq.tsv";
...
my $itol_genus = $date."_itol_genus_annotations.txt";
...
my $itol_sub = $date."_itol_subfamily_annotations.txt";
...
my $itol_family = $date."_itol_family_annotations.txt";
...
my $itol_lowest_taxa = $date."_itol_lowest_taxa_annotations.txt";
...
my $itol_host = $date."_itol_host_annotations.txt";
...
my $itol_node = $date."_itol_node_label_annotations.txt";
...
my $itol_length = $date."_itol_length_annotations.txt";
...
my $refseq = $date."_refseq_genomes.fa";
...
sub filter_genomes { 
    ...
    while (my $inseq_obj = $seq_in->next_seq) {
        ...
        ($acc =~ m/NC_002180|NC_008355|NC_002180|NC_019920|NC_019922|NC_027984|NC_028993|NC_028994|NC_028651|NC_025471|NC_026013|NC_025444|NC_029032|NC_028998|NC_004173|NC_004174|NC_004175|NC_004171|NC_004170|NC_003301|NC_003300|NC_003299|NC_003714|NC_003715|NC_003716|NC_023586|NC_021866|NC_020869|NC_026612|NC_026613|NC_024711|NC_026582|JN377905/) && ($inseq_obj->seq) && ($acc !~ m/$exclusion_list/)) {
            ...
            if ($host =~ m/^abalone$|^ANMV-1$|^Archaeal$|^Deep$|^Halocynthia$|^His$|^His2$|^IAS$|^Los$|^Marine$|^Salicola$|^Stx$|^TM$|^uncultured$/i) {
            ...
            if ($primary_id !~ m/NC_*/ ) {
                ...
                print VGENUS "$primary_id\t$genus\t$genus_hex\n";
                print VSUB "$primary_id\t$sub_family\t$sub_family_hex\n";
                print VFAMILY "$primary_id\t$family\t$family_hex\n";
                print VLOWEST "$primary_id\t$lowest_taxa\t$lowest_taxa_hex\n";
                print VHOST "$primary_id\t$host\t$host_hex\n";
